...
import de.fraunhofer.sciencedataamanager.connectors.webofscience.premium.FullRecordData;
import de.fraunhofer.sciencedataamanager.connectors.webofscience.premium.FullRecordSearchResults;
import de.fraunhofer.sciencedataamanager.connectors.webofscience.premium.LabelValuesPair;
import de.fraunhofer.sciencedataamanager.connectors.webofscience.premium.QueryParameters;
import de.fraunhofer.sciencedataamanager.connectors.webofscience.premium.RetrieveParameters;
import de.fraunhofer.sciencedataamanager.connectors.webofscience.premium.WokSearch;
import de.fraunhofer.sciencedataamanager.connectors.webofscience.premium.WokSearchService;
 ...
 * @author Moritz Mars
 */
public class WebOfScienceFullConnector implements ICloudPaperConnector {

    private ApplicationConfiguration applicationConfiguration;
    int progress = 0;
    int itemTreshhold = 50;

    public WebOfScienceFullConnector(ApplicationConfiguration applicationConfiguration) {
    ...
    public SearchDefinitonExecution getCloudPapers(SearchDefinition searchDefiniton) throws Exception {
        ...
        WokSearchService WokSearchService = new WokSearchService(urlWokSearch);
        ...
        FullRecordSearchResults results = wokSearch.search(queryParameters, retrieveParameters);
        ...
        while (true)
        {
            ...
            FullRecordData resultsData = wokSearch.retrieve(results.getQueryId(), retrieveParameters);
            this.applicationConfiguration.getLoggingManager().log("Get Records: " + resultsData.getRecords(), LogLevel.DEBUG);

            for (LabelValuesPair currentRecord : resultsData.getOptionValue())
            {
                ...
                try
                {
                    this.applicationConfiguration.getLoggingManager().log("Current Label:" + currentRecord.getLabel(), LogLevel.DEBUG);
                    this.applicationConfiguration.getLoggingManager().log("Current Value:" + currentRecord.getValue().get(0), LogLevel.DEBUG);
...
//                                }
  ...
  //                      searchDefinitonExecution.getScientificPaperMetaInformation().add(scientificPaperMetaInformation);
                ...
                catch (Exception ex)
                {
                    ...
                    //cientificPaperMetaInformationParseException.setScientificPaperMetaInformation(scientificPaperMetaInformation);
